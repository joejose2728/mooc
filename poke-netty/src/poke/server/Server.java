...
public class Server {
	...
	private void startManagers() {
		...
		electionMgr.setNearestNodes(conf.getNearest().getNearestNodes());
		...
		//TODO: Ideally the server should listen to all nodes up event?
		
		//VERSION 4.1 - wait before all servers start up
		/*try {
			Thread.sleep(120000);
	...
	class ElectionInitializer extends Thread {
		...
		public void run() {
			...
			if (!electionMgr.hasLeader()){
				//declare election
				logger.info("Declaring election....");
				electionMgr.declareElection();
			}
		}
	}
